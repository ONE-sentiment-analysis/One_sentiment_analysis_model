...
BASE_DIR = Path(__file__).parent.parent # vai para /src
...
logging.basicConfig(
    filename=LOGS_DIR / "execucao.log",
    level=logging.INFO,
...
def main():
    parser = argparse.ArgumentParser(description="CLI para Análise de Sentimento")
    parser.add_argument("-t", "--text", type=str, required=True)
    parser.add_argument("-m", "--model", type=str, required=True, choices=['nb', 'lr', 'rf'])
    args = parser.parse_args()

    modelo, vectorizer = carregar_recursos(args.model)
    
    texto_limpo = limpar_texto(args.text)
    ...
    # logando a execucao
    log_msg = f"{args.model.upper()} | Texto: '{args.text}' | Predição: {predicao} | Confiança: {probabilidade:.2%}"
    logging.info(log_msg)

    # output para o usuario
    print(f"\nResultado ({args.model.upper()}): {predicao} ({probabilidade:.2%})")
